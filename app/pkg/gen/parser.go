...
func getGormTag(tag string) string {
	...
	for _, part := range strings.Fields(tag) {
		if strings.HasPrefix(part, "gorm:") {
			return strings.Trim(strings.TrimPrefix(part, "gorm:"), "\"")
		}
	}
	return ""
}
...
func isSearchable(f *Field) bool {
	...
	switch f.Name {
	case "DeletedAt", "ID", "Password", "LoginAttempts", "LockedUntil":
		return false
	}
	return true
}
...
func isStructType(t string) bool {
	...
	switch t {
	case "Role", "Permission", "User", "File", "Task", "UserRole", "RolePermission":
		return true
	}
	return false
}
...
func getSwaggerType(goType string) string {
	...
	switch {
	case goType == "string" || goType == "time.Time":
		return "string"
	case goType == "bool":
		return "boolean"
